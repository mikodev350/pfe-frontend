...
const ResourceDetails = ({ resource }) => {
  ...
  useEffect(() => {
    const cacheResources = async () => {
      if (resource.isLocalUpload) {
        const localBlobObject = { ...blobObject };

        if (resource.images && !navigator.onLine) {
          localBlobObject.images = resource.images;
        }

        if (resource.audio && !navigator.onLine) {
          localBlobObject.audio = resource.audio;
        }

        if (resource.video && !navigator.onLine) {
          localBlobObject.video = resource.video;
        }

        if (resource.pdf && !navigator.onLine) {
          localBlobObject.pdf = resource.pdf;
        }

        setBlobObject(localBlobObject);
    ...
    cacheResources();
  }, [resource, fetchMediaFromCache, blobObject]);

  const renderMedia = (media) => {
    if (!media || !media.url) return null;
    return <Image src={media.url} alt={media.name || "Resource Image"} className="img-fluid" />;
  };
  ...
  const renderBlobImages = (image) => {
    if (!image ) return null;
    return <Image src={image} alt={ "Resource Image"} className="img-fluid" />;
      ...
      <Card.Body>
        ...
        <Row className="mt-4">
          ...
          <Col md={6}>
            ...
            {/* ***************************************************************************************************** */}
            {blobObject && (blobObject.images.length > 0 || blobObject.video || blobObject.audio || blobObject.pdf) ? (
  <div className="mt-4">
    {blobObject.images.length > 0 && (
      <>
        <h3>Images</h3>
        <Row>
          {blobObject.images.map((image, index) => (
            <Col md={4} key={index}>
              <Zoom>
                {renderBlobImages(image)}
              </Zoom>
              <Button variant="link" onClick={() => handleShow(index)}>
                Voir en plein écran
              </Button>
    ...
    {blobObject.video && renderBlobVideo()}
    {blobObject.audio && renderBlobAudio()}
    {blobObject.pdf && renderBlobPDF()}
  </div>
) : (
